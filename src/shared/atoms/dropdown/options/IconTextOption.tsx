...
export const createIconTextOptions = (
  options: Array<{ value: string; icon: React.ReactElement<typeof Icon>; text: string }>
): DropdownOption<string>[] =>
  options.map(opt => createIconTextOption(opt.value, opt.icon, opt.text));


const extractIconFromContent = (content: React.ReactNode): React.ReactElement<typeof Icon> | null => {
